...
/// @brief minifying function to check if the left button is CURRENTLY being pushed, 
/// strictly in the instantaneous moment
bool isLeftButtonActive() { return (digitalRead(pin_btn_left) == LOW); }

/// @brief minifying function to check if the right button is CURRENTLY being pushed, 
/// strictly in the instantaneous moment
bool isRightButtonActive() { return (digitalRead(pin_btn_right) == LOW); }

/// @brief One of two outward-facing function of this lib.  
/// Robustly checks the "left" button to provide means of
/// distinguishing debounce events from genuine human input
bool isLeftReceivingPureInput() {
  if (isLeftButtonActive()) {     // is button peing pushed?
...
bool isRightReceivingPureInput() {
  if (isRightButtonActive()) {     // is button peing pushed?
    if (millis() - msSinceLastPress > DEBOUNCE_DELAY_MS) {
        // if so, has it been a *reasonable* amount of time?
        msSinceLastPress = millis();    // reset time of press
        return true;
